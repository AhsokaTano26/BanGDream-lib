...
export type CalendarFilter = 'all' | 'birthday' | 'event'

const CAL_NAME: Record<CalendarFilter, string> = {
  all: 'BanG Dream! 同好会',
  birthday: 'BanG Dream! 生日',
  event: 'BanG Dream! 活动',
}

// --- Build ICS ---

export const buildICS = async (filter: CalendarFilter): Promise<string> => {
  const needEvents = filter === 'all' || filter === 'event'
  const needBirthdays = filter === 'all' || filter === 'birthday'

  const [blogPosts, mediaPosts, newsPosts, discPosts, birthdays] = await Promise.all([
    needEvents ? readCollection('blog', 'date', 'status') : Promise.resolve([]),
    needEvents ? readCollection('media', 'date', 'type') : Promise.resolve([]),
    needEvents ? readCollection('news', 'date', 'status') : Promise.resolve([]),
    needEvents ? readCollection('discographies', 'date', 'status') : Promise.resolve([]),
    needBirthdays ? readBirthdays() : Promise.resolve([]),
  ])
  ...
  lines.push(foldLine(`X-WR-CALNAME:${escapeIcal(CAL_NAME[filter])}`))
